...
module aes128_encrypt (
    ...
    function [31:0] sub_word; input [31:0] w; begin sub_word={sbox(w[31:24]),sbox(w[23:16]),sbox(w[15:8]),sbox(w[7:0])}; end endfunction
    function [31:0] rot_word; input [31:0] w; begin rot_word={w[23:0],w[31:24]}; end endfunction
    function [31:0] rcon; input [3:0] i; begin case(i) 4'd1:rcon=32'h01000000;4'd2:rcon=32'h02000000;4'd3:rcon=32'h04000000;4'd4:rcon=32'h08000000;4'd5:rcon=32'h10000000;4'd6:rcon=32'h20000000;4'd7:rcon=32'h40000000;4'd8:rcon=32'h80000000;4'd9:rcon=32'h1b000000;4'd10:rcon=32'h36000000;default:rcon=32'h0; endcase end endfunction
    function [127:0] next_key; input [127:0] k; input [3:0] r; reg [31:0] w0,w1,w2,w3,t; begin w0=k[127:96];w1=k[95:64];w2=k[63:32];w3=k[31:0];t=sub_word(rot_word(w3))^rcon(r);w0=w0^t;w1=w1^w0;w2=w2^w1;w3=w3^w2;next_key={w0,w1,w2,w3}; end endfunction
    function [127:0] sub_bytes; input [127:0] s; integer i; begin for(i=0;i<16;i=i+1) sub_bytes[127-i*8 -: 8]=sbox(s[127-i*8 -: 8]); end endfunction
    function [127:0] shift_rows; input [127:0] s; reg [7:0] b[0:15]; integer i; begin for(i=0;i<16;i=i+1) b[i]=s[127-i*8 -: 8]; shift_rows={b[0],b[5],b[10],b[15],b[4],b[9],b[14],b[3],b[8],b[13],b[2],b[7],b[12],b[1],b[6],b[11]}; end endfunction
    function [31:0] mix_col; input [31:0] c; reg [7:0] a0,a1,a2,a3; begin a0=c[31:24];a1=c[23:16];a2=c[15:8];a3=c[7:0]; mix_col={xtime(a0)^(xtime(a1)^a1)^a2^a3, a0^xtime(a1)^(xtime(a2)^a2)^a3, a0^a1^xtime(a2)^(xtime(a3)^a3), (xtime(a0)^a0)^a1^a2^xtime(a3)}; end endfunction
    function [127:0] mix_columns; input [127:0] s; begin mix_columns={mix_col(s[127:96]),mix_col(s[95:64]),mix_col(s[63:32]),mix_col(s[31:0])}; end endfunction
    reg [127:0] state_reg;
    reg [127:0] round_key_reg;
    reg [3:0] round;
    reg busy;

    reg [127:0] next_round_key_reg;
    reg [127:0] round_state;

    always @(*) begin
        next_round_key_reg = next_key(round_key_reg, round);

        if (round == 4'd10)
            round_state = shift_rows(sub_bytes(state_reg)) ^ next_round_key_reg;
        else
            round_state = mix_columns(shift_rows(sub_bytes(state_reg))) ^ next_round_key_reg;
    end
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (start && !busy) begin
                ...
                round_key_reg <= next_round_key_reg;
